...
uint16_t adc1_dma_buffer[100];
uint16_t adc2_dma_buffer[100];
...
int main(void)
{
  ...
  HAL_DAC_SetValue(&hdac1, DAC_CHANNEL_2, DAC_ALIGN_12B_R, 2583); // ~2200  mV 2733
  HAL_DAC_SetValue(&hdac1, DAC_CHANNEL_1, DAC_ALIGN_12B_R, 2853); // ~2418 mV 3003
  // Turn on safety
  HAL_GPIO_WritePin(SAFETY_GPIO_Port, SAFETY_Pin, GPIO_PIN_SET);
  // APPS
  HAL_ADC_Start_DMA(&hadc1, reinterpret_cast<uint32_t*>(adc1_dma_buffer), 50);
  HAL_ADC_Start_DMA(&hadc2, reinterpret_cast<uint32_t*>(adc2_dma_buffer), 50);
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  int apps_value_to_send = apps.get_value_to_send();
